...
public:
    ...
    mbgl::style::PropertyValue<MBGLType> toInterpolatablePropertyValue(MGLStyleValue<ObjCType> *value) {
        ...
        if ([value isKindOfClass:[MGLStyleConstantValue class]]) {
            ...
            switch (cameraStyleFunction.stopType) {
                case MGLStyleFunctionStopTypeExponential:
    ...
    mbgl::style::DataDrivenPropertyValue<MBGLType> toDataDrivenPropertyValue(MGLStyleValue<ObjCType> *value) {
        if ([value isKindOfClass:[MGLStyleConstantValue class]]) {
            ...
            switch (sourceStyleFunction.stopType) {
                case MGLStyleFunctionStopTypeExponential:
                ...
                case MGLStyleFunctionStopTypeInterval:
                    ...
                    break;
                case MGLStyleFunctionStopTypeCategorical:
                    return toMBGLCategoricalSourceFunction(sourceStyleFunction);
                    break;
                case MGLStyleFunctionStopTypeIdentity:
            ...
            switch (compositeStyleFunction.stopType) {
                case MGLStyleFunctionStopTypeExponential: {
...
private: // Private utilities for converting from mbgl to mgl values
    ...
    public:
        id operator()(const mbgl::style::ExponentialStops<MBGLType> &mbglStops) {
            return [MGLCameraStyleFunction functionWithStopType:MGLStyleFunctionStopTypeExponential
        ...
        id operator()(const mbgl::style::IntervalStops<MBGLType> &mbglStops) {
            return [MGLCameraStyleFunction functionWithStopType:MGLStyleFunctionStopTypeInterval
                                                          stops:toConvertedStops(mbglStops.stops)
                                                        options:nil];
        }
    };
    ...
    public:
        id operator()(const mbgl::style::ExponentialStops<MBGLType> &mbglStops) {
            return [MGLSourceStyleFunction functionWithStopType:MGLStyleFunctionStopTypeExponential
                                                          stops:toConvertedStops(mbglStops.stops)
        ...
        id operator()(const mbgl::style::IntervalStops<MBGLType> &mbglStops) {
            return [MGLSourceStyleFunction functionWithStopType:MGLStyleFunctionStopTypeInterval
                                                          stops:toConvertedStops(mbglStops.stops)
                                                  attributeName:@(mbglFunction.property.c_str())
                                                        options:nil];
        }
        ...
        id operator()(const mbgl::style::CategoricalStops<MBGLType> &mbglStops) {
            ...
            return [MGLSourceStyleFunction functionWithStopType:MGLStyleFunctionStopTypeCategorical
                                                          stops:stops attributeName:@(mbglFunction.property.c_str())
                                                        options:@{MGLStyleFunctionOptionDefaultValue: defaultValue}];
        }

        id operator()(const mbgl::style::IdentityStops<MBGLType> &mbglStops) {
            return [MGLSourceStyleFunction functionWithStopType:MGLStyleFunctionStopTypeIdentity
                                                          stops:nil
    ...
    public:
        id operator()(const std::map<float, mbgl::style::ExponentialStops<MBGLType>> &mbglStops) {
            ...
            return [MGLCompositeStyleFunction functionWithStopType:MGLStyleFunctionStopTypeExponential
                                                             stops:stops
                                                     attributeName:@(mbglFunction.property.c_str())
        ...
        id operator()(const std::map<float, mbgl::style::IntervalStops<MBGLType>> &mbglStops) {
            ...
            return [MGLCompositeStyleFunction functionWithStopType:MGLStyleFunctionStopTypeInterval
                                                             stops:stops
                                                     attributeName:@(mbglFunction.property.c_str())
                                                           options:nil];
        }
        ...
        id operator()(const std::map<float, mbgl::style::CategoricalStops<MBGLType>> &mbglStops) {
            MGLStyleValue<ObjCType> *defaultValue;
            NSMutableDictionary *stops = [NSMutableDictionary dictionaryWithCapacity:mbglStops.size()];
            for (auto const& outerStop: mbglStops) {
                ...
                for (const auto &mbglStop : outerStop.second.stops) {
                    ...
                    defaultValue = [MGLStyleValue valueWithRawValue:toMGLRawStyleValue(outerStop.second.defaultValue)];
                }
                stops[@(outerStop.first)] = innerStops;
            }

            return [MGLCompositeStyleFunction functionWithStopType:MGLStyleFunctionStopTypeCategorical
                                                             stops:stops attributeName:@(mbglFunction.property.c_str())
                                                           options:@{MGLStyleFunctionOptionDefaultValue: defaultValue}];
        }
